...
const dec = (v) => (v === undefined || v === null ? undefined : new client_1.Prisma.Decimal(v));
let OpticsService = class OpticsService {
    prisma;
    audit;
    inventario;
    timeline;
    constructor(prisma, audit, inventario, timeline) {
        this.prisma = prisma;
        this.audit = audit;
        this.inventario = inventario;
        this.timeline = timeline;
    }
    async emitirFormula(datos, ctx) {
        for (const [ojo, eje, cil] of [
            ...
            if (eje !== undefined && (eje < 0 || eje > 180)) {
                throw new common_1.BadRequestException(`El eje del ojo ${ojo} debe estar entre 0 y 180 grados.`);
            }
            if (cil !== undefined && cil !== 0 && eje === undefined) {
                throw new common_1.BadRequestException(`Hay cilindro en el ojo ${ojo} pero no hay eje.`);
            }
        }
    ...
    async crearOrden(datos, ctx) {
        const f = await this.prisma.prescription.findUnique({
            where: { id: datos.prescriptionId },
            select: { id: true, personId: true, validTo: true },
        });
        if (!f)
            throw new common_1.NotFoundException('Fórmula no encontrada');
        if (f.validTo && f.validTo < new Date()) {
            throw new common_1.BadRequestException(`Esa fórmula venció el ${f.validTo.toISOString().slice(0, 10)}. Tome una nueva antes de pedir los lentes.`);
        }
        if (!datos.frameProductId && !datos.frameOwn && !datos.frameNote) {
            throw new common_1.BadRequestException('Indique la montura: del inventario, propia del paciente o descrita.');
        }
    ...
    async enviarALaboratorio(id, lab, promisedAt, ctx) {
        const o = await this.prisma.opticalOrder.findUnique({ where: { id }, select: { status: true } });
        if (!o)
            throw new common_1.NotFoundException('Orden no encontrada');
        if (o.status !== 'TOMADA')
            throw new common_1.BadRequestException('Ya salió al laboratorio o está más adelante.');
    ...
    async recibirDeLaboratorio(id, ctx) {
        const o = await this.prisma.opticalOrder.findUnique({ where: { id }, select: { status: true } });
        if (!o)
            throw new common_1.NotFoundException('Orden no encontrada');
        if (o.status !== 'EN_LABORATORIO')
            throw new common_1.BadRequestException('No está en laboratorio.');
        return this.prisma.opticalOrder.update({
            where: { id },
    ...
    async entregar(id, deliveredTo, ctx) {
        ...
        if (o.status === 'ENTREGADA')
            throw new common_1.BadRequestException('Ya fue entregada.');
        if (o.status !== 'RECIBIDA') {
            throw new common_1.BadRequestException('Todavía no llegó del laboratorio: no se puede entregar.');
        }
        for (const productId of [o.frameProductId, o.lensProductId].filter(Boolean)) {
    ...
    async anular(id, motivo, ctx) {
        ...
        if (!o)
            throw new common_1.NotFoundException('Orden no encontrada');
        if (o.status === 'ENTREGADA') {
            throw new common_1.BadRequestException('Ya fue entregada: registre una garantía o una devolución, no una anulación.');
        }
    ...
    async ordenes(filtro) {
        ...
        return ordenes.map((o) => ({
            ...o,
            diasDeAtraso: o.promisedAt && !o.deliveredAt && o.promisedAt < hoy
...
exports.OpticsService = OpticsService;
exports.OpticsService = OpticsService = __decorate([
...
//# sourceMappingURL=optics.service.js.map