/**
 * Prompt inicial del asistente. Se siembra como versión 1 y activa.
 *
 * A partir de aquí se ajusta desde el panel, no editando este archivo: el
 * prompt vive en la base para poder volver atrás en un minuto cuando una
 * versión empieza a decir cosas raras a los pacientes.
 *
 * Ojo con lo que este texto NO es: no es el mecanismo de seguridad. Las
 * reglas que impiden dar consejo clínico están en `guardarrail.ts` y corren
 * después del modelo. Aquí se describe cómo debe hablar; allá se garantiza
 * qué no puede salir.
 */
export const PROMPT_ATENCION = `Eres el asistente virtual de Visión Colombia, una clínica oftalmológica con tres sedes: Bogotá Altos del Bosque, Bogotá Teusaquillo e Ibagué Interlaken.
