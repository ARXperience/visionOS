/**
 * Datos de ejemplo para poder ver la agenda funcionando en desarrollo.
 *
 *   SEED_DEMO=1 npm run seed --workspace apps/api
 *
 * Está detrás de una bandera y NO se ejecuta por defecto. Profesionales
 * inventados en la base de una clínica son un riesgo real: alguien agenda un
 * paciente con la "Dra. Prueba" y ese paciente se presenta un martes a las
 * nueve. Todos llevan el prefijo DEMO- para poder borrarlos de un golpe.
 */
import type { PrismaClient } from '@prisma/client';

const PROFESIONALES = [
  { doc: 'DEMO-1', nombre: 'Laura', apellido: 'Restrepo', tipo: 'OFTALMOLOGO' as const, esp: ['RETINA'], color: '#0E93B4' },
  { doc: 'DEMO-2', nombre: 'Andrés', apellido: 'Gómez', tipo: 'OFTALMOLOGO' as const, esp: ['GLAUCOMA', 'CORNEA'], color: '#1554A8' },
  { doc: 'DEMO-3', nombre: 'Carolina', apellido: 'Ríos', tipo: 'OPTOMETRA' as const, esp: [], color: '#7A5B10' },
];
...
export async function sembrarDemo(prisma: PrismaClient): Promise<void> {
  ...
  if (!sedes.length) throw new Error('Siembre las sedes primero');
  ...
  for (const p of PROFESIONALES) {
    ...
    // Atienden en todas las sedes, lunes a viernes de 8 a 12 y de 14 a 18.
    for (const sede of sedes) {
    ...
    // Habilitados para consultas y exámenes. Las cirugías se dejan fuera a
    // propósito: no se inventa quién opera.
  ...
  console.log(
    `demo: ${PROFESIONALES.length} profesionales, ${CONSULTORIOS.length} salas por sede, ` +
