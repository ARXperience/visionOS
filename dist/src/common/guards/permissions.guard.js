...
let PermissionsGuard = class PermissionsGuard {
    ...
    canActivate(context) {
        ...
        const requeridos = this.reflector.getAllAndOverride(require_permission_decorator_1.PERMISSIONS_KEY, [
            context.getHandler(),
            context.getClass(),
        ]);
        ...
        const efectivos = (0, permissions_1.permissionsOf)(user);
        const faltantes = requeridos.filter((p) => !efectivos.has(p));
        if (faltantes.length) {
            throw new common_1.ForbiddenException(`Falta el permiso: ${faltantes.join(', ')}`);
        }
        return true;
    }
};
